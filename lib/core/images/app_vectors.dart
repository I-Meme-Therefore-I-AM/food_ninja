class AppVectors {
  static const basePath = 'assets/vectors/';
  static const ext = 'svg';

  static const logo = '${basePath}logo.$ext';
  static const pattern = '${basePath}pattern.$ext';
  static const eateryBg = '${basePath}eatery.$ext';
  static const eateryBg_2 = '${basePath}eatery_2.$ext';

  // social auth icon
  static const facebook = '${basePath}facebook.$ext';
  static const google = "${basePath}google.$ext";

  // sign up icons
  static const profile = '${basePath}profile.$ext';
  static const lock = '${basePath}lock.$ext';
  static const message = '${basePath}message.$ext';
}
